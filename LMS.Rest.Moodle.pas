...
var
  ...
  filename: string;
begin
  filename := aFilename + '&token=' + fToken;
  ares := URLDownloadToFile(nil, pchar(filename), pchar(path), 0, nil);
  if ares = S_OK then
    Log('Downloaded ' + aFilename + ' to ' + path)
  else
  begin
    Log('----- err:' + ares.ToString);
    Log(aFilename + '&token=' + fToken);
    Log(path);
    Log('-----');
  end;
end;
